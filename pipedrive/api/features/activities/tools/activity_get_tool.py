...
async def get_activity_from_pipedrive(
    ...
    Format requirements:
    - id_str: Activity ID as a numeric string (e.g., "123")
    ...
    get_activity_from_pipedrive(
        id_str="123",
        include_fields_str="attendees"
    )
    ```

    Args:
        ctx: Context object provided by the MCP server
        id_str: ID of the activity to retrieve
        include_fields_str: Optional comma-separated list of additional fields to include
    ...
    # Sanitize inputs
    inputs = {"id": id_str, "include_fields": include_fields_str}
