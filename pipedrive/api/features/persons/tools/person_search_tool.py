...
async def search_persons_in_pipedrive(
    ...
    Format requirements:
        ...
        - limit_str: Maximum number of results to return (max 100)
    ...
    try:
        ...
        if limit < 1:
            ...
            return format_tool_response(False, error_message=error_msg)
        if limit > 100:
            logger.warning(f"Search limit {limit} exceeds Pipedrive API max (100). Capping to 100.")
            limit = 100
    except ValueError:
    ...
    try:
        ...
        while True:
            ...
            all_results.extend(search_results or [])
            pages_fetched += 1
        ...
        logger.info(
            f"Found {len(all_results)} persons matching '{term}' "
